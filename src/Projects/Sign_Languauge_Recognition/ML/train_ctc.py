"""
Pure CTC Training (no seq2seq decoder interference)
"""
import os
import torch
...
from data_pipeline import train_loader, val_loader, vocab_size, id_to_token
...
# Pure CTC model - no seq2seq decoder needed
model = Seq2Seq(
    input_dim=INPUT_DIM,
    vocab_size=cfg.vocab_size,
    hidden_dim=cfg.hidden_dim,
    use_ctc=True,  # Only CTC head active
).to(device)
...
def prepare_ctc_targets(tgt, cfg):
    """Prepare CTC targets: flatten and remove pad/sos/eos"""
    targets = []
    lengths = []
...
for epoch in range(cfg.max_epochs):
    ...
    for src, src_mask, tgt in train_loader:
        ...
        ctc_logits = model.ctc_head(memory)  # [B, T, V+1]
    ...
    if val_loss < best_val_loss and val_loss > 0:
        ...
        }, "checkpoints/ctc_best.pt")
        print("Saved checkpoints/ctc_best.pt")